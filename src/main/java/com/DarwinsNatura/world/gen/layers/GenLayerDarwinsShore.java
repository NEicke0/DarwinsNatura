...
import com.google.common.base.Predicate;
	...
	public class GenLayerDarwinsShore extends GenLayerBase
	{
	    public GenLayerDarwinsShore(long seed, GenLayer parent)
	    {
	        super(seed);
	        this.parent = parent;
	    }
	    ...
	    public int[] getInts(int areaX, int areaY, int areaWidth, int areaHeight)
	    {
	        ...
	        for (int z = 0; z < areaHeight; ++z)
	        {
	            for (int x = 0; x < areaWidth; ++x)
	            {
	                ...
	                if (biomeId == Biome.getIdForBiome(DarwinsNaturaBiomes.GALAPAGOS))
	                {
	                    setBiomeWithAdjacent(biomeIds, out, x, z, areaWidth, biomeId, Biome.getIdForBiome(Biomes.BEACH), OCEAN_PREDICATE);
	                }
	                    ...
	                    else if (biomeId != Biome.getIdForBiome(Biomes.MESA) && biomeId != Biome.getIdForBiome(Biomes.MESA_ROCK))
	                    {
	                        if (biomeId != Biome.getIdForBiome(Biomes.OCEAN) && biomeId != Biome.getIdForBiome(Biomes.DEEP_OCEAN) && biomeId != Biome.getIdForBiome(Biomes.RIVER) && biomeId != Biome.getIdForBiome(Biomes.SWAMPLAND))
	                        {
	                            ...
	                            if (biome != null)
	                            {
	                                ...
	                                setBiomeWithAdjacent(biomeIds, out, x, z, areaWidth, biomeId, biome == null ? biomeId : Biome.getIdForBiome(Biomes.BEACH), OCEANIC_PREDICATE);
	                            }
	                            else
	                            {
	                                setBiomeWithAdjacent(biomeIds, out, x, z, areaWidth, biomeId, Biome.getIdForBiome(Biomes.BEACH), OCEANIC_PREDICATE);
